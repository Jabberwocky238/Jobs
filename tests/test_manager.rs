...
use Jobs::{JManager, ManagerAction, ManagerStorage};
...
fn warpper(TEMP_DIR: &str) -> Result<(), Box<dyn Error>> {
    ...
    manager.update_node(&locate)?;
    let info = manager.get_info(&locate)?;
    assert_eq!(info.count_dir, 30);
    assert_eq!(info.count_file, 125);

    // dump，
    manager.dump()?;

    // 添加一个文件夹，删掉一个文件夹，
    fs::remove_dir_all(format!("{TEMP_DIR}/DIR_1_0"))?;
    ...
    let info = manager.get_info(&locate)?;
    let size = info.size;
    assert_eq!(info.count_dir, 26);
    assert_eq!(info.count_file, 105);
    ...
    let locate = manager.locate_node(&PathBuf::from(TEMP_DIR))?;
    let info = manager.get_info(&locate)?;
    assert_eq!(info.count_dir, 21);
    assert_eq!(info.count_file, 80);
